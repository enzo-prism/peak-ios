...
enum WaveStatsCalculator {
    ...
    static func records(sessions: [SurfSession]) -> WaveRecords {
        ...
        // Ties break toward the more recent session: a record you just matched
        // feels like yours, and it keeps the row changing as you keep surfing.
        if let best = sessions
            .filter({ ($0.waveCount ?? 0) > 0 })
            .max(by: { lhs, rhs in
                let l = lhs.waveCount ?? 0, r = rhs.waveCount ?? 0
                return l == r ? lhs.date < rhs.date : l < r
            }), let count = best.waveCount {
        ...
        if let best = sessions
            .filter({ ($0.topSpeedKph ?? 0) > 0 })
            .max(by: { lhs, rhs in
                let l = lhs.topSpeedKph ?? 0, r = rhs.topSpeedKph ?? 0
                return l == r ? lhs.date < rhs.date : l < r
            }), let speed = best.topSpeedKph {
            let formatted = WaveStatsFormatter.speed(speed)
            records.fastestWave = record(for: best, value: formatted, spokenValue: formatted)
        }

        if let best = sessions
            .filter({ ($0.longestRideSeconds ?? 0) > 0 })
            .max(by: { lhs, rhs in
                let l = lhs.longestRideSeconds ?? 0, r = rhs.longestRideSeconds ?? 0
                return l == r ? lhs.date < rhs.date : l < r
            }), let seconds = best.longestRideSeconds {
